...
export async function POST(request: NextRequest) {
  try {
    const { chainId, bet, epoch, player, recipient } = await request.json();
    ...
    const { contractAddresses: ca, transactionData: td} = filterTransactionData({chainId, filter:true, functionNames: ['withdraw']});
    ...
    const hash = await walletClient.writeContract({
      ...
      args: [BigInt(bet), player, BigInt(epoch), recipient]
    });
